 ...
 */
 ...
 // ----------------------
 // This file is modified for compability with Hadoop Streaming (Python) 3.2.1
 // Modified by 42Inc (Alexey R.), 2020 
 //
...
public class XmlInputFormat extends FileInputFormat {
    ...
    @Override
    public RecordReader<Text, Text> getRecordReader(final InputSplit split, final JobConf job,final Reporter report) {
      final Configuration conf = new Configuration();
      conf.addResource(new Path("file:///opt/hadoop/etc/hadoop/core-site.xml")); // Replace with actual path
      conf.addResource(new Path("file:///opt/hadoop/etc/hadoop/hdfs-site.xml")); // Replace with actual path
...
//    }
    ...
    * 
    */
    public static class XmlRecordReader implements /*extends*/ RecordReader<Text, Text> {

        private final byte[] keyStartTag;
        private final byte[] keyEndTag;
        private final byte[] valueStartTag;
        private final byte[] valueEndTag;
        private final byte[] _keyStartTag;
        private final byte[] _keyEndTag;
        ...
        public XmlRecordReader(final FileSplit split, final Configuration conf) throws IOException {
            ...
            _keyStartTag = "<uniqid>".getBytes(Charsets.UTF_8);
            _keyEndTag = "</uniqid>".getBytes(Charsets.UTF_8);
        ...
        //
        // Output:
        //   key: string "<id>key</id>" -> "key<uniqid>"
        //   value: string "<text>value</text>" -> "value"
        //
        @Override
...
//            key_out.set(_keyStartTag, 0, _keyStartTag.length);
            if (fsin.getPos() < end && readUntilMatch(keyStartTag, false)) {
                // Read article id (<id>id</id>)
                try {
                    if (readUntilMatch(keyEndTag, true)) {
                        key_out.set(buffer.getData(), 0, buffer.getLength() - keyEndTag.length);
                        key_out.append(_keyStartTag, 0, _keyStartTag.length);
//                        key_out.append(_keyEndTag, 0, _keyEndTag.length);
                        key.set(key_out.toString().getBytes(Charsets.UTF_8), 0, key_out.getLength());
